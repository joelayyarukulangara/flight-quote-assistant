...
from config import (
    ...
    BAGGAGE_OPTIONS, RETURN_CALC_RULES, AI_MODELS, AI_LANGUAGES,
)
...
class SettingsView(ctk.CTkScrollableFrame):
    ...
    def _build_form(self):
        ...
        ctk.CTkLabel(api_frame, text="API KEYS", font=ctk.CTkFont(size=15, weight="bold")).grid(
            row=0, column=0, sticky="w", padx=10, pady=(10, 5)
        )

        ctk.CTkLabel(api_frame, text="SerpAPI API key (leave blank to use mock data)").grid(row=1, column=0, sticky="w", padx=10)
        self.serpapi_key_entry = ctk.CTkEntry(api_frame, show="*")
        self.serpapi_key_entry.grid(row=2, column=0, sticky="ew", padx=10, pady=(0, 8))

        ctk.CTkLabel(api_frame, text="OpenAI API key (optional, for AI quote wording)").grid(row=3, column=0, sticky="w", padx=10)
        self.openai_key_entry = ctk.CTkEntry(api_frame, show="*")
        self.openai_key_entry.grid(row=4, column=0, sticky="ew", padx=10, pady=(0, 8))

        self.mock_mode_var = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(api_frame, text="Enable mock mode (always use sample data, ignore live API)",
                        variable=self.mock_mode_var).grid(row=5, column=0, sticky="w", padx=10, pady=(0, 10))
        ...
        ctk.CTkLabel(ai_frame, text="AI SUMMARY (OPTIONAL)", font=ctk.CTkFont(size=15, weight="bold")).grid(
            row=0, column=0, columnspan=2, sticky="w", padx=10, pady=(10, 5)
        )

        self.enable_ai_var = ctk.BooleanVar(value=False)
        ctk.CTkCheckBox(ai_frame, text="Enable AI summary (requires OpenAI API key)",
                        variable=self.enable_ai_var).grid(row=1, column=0, columnspan=2, sticky="w", padx=10, pady=(0, 8))

        ctk.CTkLabel(ai_frame, text="AI model").grid(row=2, column=0, sticky="w", padx=10)
        self.ai_model_menu = ctk.CTkOptionMenu(ai_frame, values=list(AI_MODELS.keys()))
        self.ai_model_menu.grid(row=3, column=0, sticky="ew", padx=10, pady=(0, 8))

        ctk.CTkLabel(ai_frame, text="AI language").grid(row=2, column=1, sticky="w", padx=10)
        self.ai_language_menu = ctk.CTkOptionMenu(ai_frame, values=AI_LANGUAGES)
        self.ai_language_menu.grid(row=3, column=1, sticky="ew", padx=10, pady=(0, 8))

        note = ctk.CTkLabel(
            ai_frame,
            text="Note: AI is used only for customer-facing wording after search results are ready.\n"
                 "It never fetches or invents flight fares. If no key is set, a rule-based quote is used instead.",
            text_color="gray", justify="left", wraplength=650,
        )
        note.grid(row=4, column=0, columnspan=2, sticky="w", padx=10, pady=(0, 10))
    ...
    def _load_settings(self):
        ...
        self.mock_mode_var.set(settings.get("enable_mock_mode", "true") == "true")
        ...
        self.ai_model_menu.set(settings.get("ai_model", "nano / cheapest"))
    ...
    def _save(self):
        ...
        if self.enable_ai_var.get() and not openai_key:
            messagebox.showwarning("AI summary disabled", "OpenAI API key is missing; AI summary will stay disabled.")

        values = {
            "serpapi_api_key": self.serpapi_key_entry.get().strip(),
            "openai_api_key": openai_key,
            "enable_mock_mode": "true" if self.mock_mode_var.get() else "false",
            ...
            "ai_model": self.ai_model_menu.get(),
